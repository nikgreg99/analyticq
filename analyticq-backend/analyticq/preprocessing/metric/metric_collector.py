...
class CodebaseMetricsCollector:
    ...
    def add_file_statistics(self, file_path: Path, language: str, size: int, loc: int):
        size = file_path.stat().st_size

        self.language_stats[language]["count"] += 1
        self.language_stats[language]["total_size"] += size
        self.language_stats[language]["files"].append({
            "file_path": PathUtil.path_to_str(file_path),
            "loc": loc
        })
        self.total_files += 1
