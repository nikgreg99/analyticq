...
class SpotBugsParser(AnalyticQResultParser):
    """Parser for SpotBugs SARIF format scan results."""

    def __init__(self):
        # Map internal field names to SARIF structure paths
        field_mapping = {
            "rule_id": "ruleId",
        ...
        self.severity_map = {
            ...
            "note": AnalyticQSeverity.LOW,
            "info": AnalyticQSeverity.INFO,
        }

        super().__init__(tool_name="spotbugs", field_mapping=field_mapping)
    ...
    def transform_sarif_to_issues(self, raw_result: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise ScanParserException(
                f"Missing field while parsing SpotBugs SARIF results: {str(e)}"
            ) from e
        except Exception as e:
            raise ScanParserException(
                f"Unexpected error transforming SpotBugs SARIF results: {str(e)}"
            ) from e
    ...
    def _transform_single_issue(self, result: Dict[str, Any], rule_details: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "message_arguments": result.get("message", {}).get("arguments", [])
        }
        ...
        return {
            ...
            "snippet": "",  # SpotBugs SARIF doesn't typically include code snippets
            "metadata": metadata
        }
    ...
    def _extract_location_info(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with uri, startLine, and endLine information
        """
        location = {
            "uri": "",
            "startLine": 0,
    ...
    def parse_scan_result(self, raw_result: Dict[str, Any]) -> AnalyticQSASTScanResult:
        ...
        try:
            ...
            raise ScanParserException(
                f"Unexpected error parsing SpotBugs SARIF results: {str(e)}"
            ) from e
