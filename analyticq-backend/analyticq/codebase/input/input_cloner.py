...
class InputClonerProtocolType(Enum):  # User for remote codebase
    HTTP = "http"
    HTTPS = "https"
    SSH = "ssh"
...
class InputCloner:
    ...
    def _get_protocol(self, codebase_url: str):
        ...
        if not scheme:
            return InputClonerProtocolType.SSH
        if scheme in ["http", "https"]:
            return InputClonerProtocolType(scheme)
        return InputClonerProtocolType.HTTPS
    ...
    async def clone_remote_codebase(self, codebase_url: str, branch: str = None, credentials: str = None):
        ...
        try:
            await asyncio.to_thread(Repo.clone_from, codebase_url, repo_path, branch=branch)
