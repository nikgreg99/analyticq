...
import pytest
from analyticq.engine import AnalyticQSASTIssueModel, AnalyticQSeverity
...
async def test_get_issue_success(issue_service, issue_repository):
    ...
    issue_repository.get_issue_by_id.return_value = AnalyticQSASTIssueModel(**TEST_ISSUE_DATA)
    ...
    assert result.id == 1
    issue_repository.get_issue_by_id.assert_called_once_with(1)
...
async def test_get_issue_not_found(issue_service, issue_repository):
    ...
    issue_repository.get_issue_by_id.return_value = None
...
async def test_create_issue_success(issue_service, issue_repository):
    ...
    """
    # Mock the repository to return a valid issue
    issue_repository.add_issue.return_value = AnalyticQSASTIssueModel(**TEST_ISSUE_DATA)
    ...
    assert isinstance(result, AnalyticQSASTIssueModel)
    assert result.id == 1
    issue_repository.add_issue.assert_called_once_with(issue_data)
...
async def test_update_issue_success(issue_service, issue_repository):
    ...
    """
    # Mock the repository to return a valid issue
    issue_repository.update_issue_by_id.return_value = AnalyticQSASTIssueModel(**TEST_ISSUE_DATA)
    ...
    issue_repository.update_issue_by_id.assert_called_once_with(1, severity=AnalyticQSeverity.HIGH)
...
async def test_delete_issue_success(issue_service, issue_repository):
    ...
    """
    # Mock the repository to return None
    issue_repository.delete_issue_by_id.return_value = AnalyticQSASTIssueModel(**TEST_ISSUE_DATA)

    # Call the service method
    await issue_service.delete_issue(1)

    # Assert the repository method was called
    issue_repository.delete_issue_by_id.assert_called_once_with(1)
...
async def test_delete_issue_failure(issue_service, issue_repository):
    ...
    issue_repository.delete_issue_by_id.side_effect = Exception("Database error")
